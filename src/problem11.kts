//credit to https://github.com/francisvalero/euler-in-kotlin
...
fun crawl(inc: Int, adj: Int, indices: List<Int>, tbl: List<Int>) =
        indices.map { idx ->
            tbl[idx] * (1..adj).map { tbl[idx + it * inc] }.fold(1) { a, b: Int -> a * b}
        }
...
fun maxProduct(adj: Int, dim: Int, list: List<Int>): Int? {
    ...
    val southE = crawl(se, adj, indices.filter(eastBoundary)
            .filter(southBoundary), list)

    val southW = crawl(sw, adj, indices.filter(westBoundary)
            .filter(southBoundary), list)

    return (south + east + southE + southW).max()
}
